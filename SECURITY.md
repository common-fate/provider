# Security

Security details, including our vulnerability disclosure and release verification processes, are listed in our documentation [here](https://docs.commonfate.io/granted/security).

If you have any questions or would like to report a vulnerability you can email us at security@commonfate.io. You may encrypt your message with our PGP key printed below.

```
-----BEGIN PGP PUBLIC KEY BLOCK-----


0SsBO8P4Ruyj7bQM4rBr2MfHwXvELvWUcdac
=vDVd
-----END PGP PUBLIC KEY BLOCK-----
```
